...
The above example will send a header called `X-Token` with the value read from the setting `my.application.token` in `the application configuration file (or the environnment variable `MY_APPLICATION_TOKEN`).
