...
The minimum requirement to configure Kafka is set the value of the `kafka.bootstrap.servers` property in the application configuration file:

[configuration, title = 'Configuring Kafka']
----
kafka:
    bootstrap:
...
[configuration, title = 'Disabling Kafka']
----
kafka:
  enabled: false
----
