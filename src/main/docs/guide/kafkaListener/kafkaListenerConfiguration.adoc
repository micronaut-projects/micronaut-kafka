...
There are a number of ways to pass configuration properties to the `KafkaConsumer`. You can set default consumer properties using `kafka.consumers.default` in the application configuration file:
...
[configuration, title = 'Applying Consumer Group Specific config']
----
kafka:
    consumers:
        myGroup:
...
You can, however, explicitly override the `Deserializer` used by providing the appropriate configuration in the application configuration file:
