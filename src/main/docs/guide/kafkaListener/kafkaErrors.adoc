...
- `LOG_AND_RESUME_AT_NEXT_RECORD` - This strategy will publish the failed record to a dead letter topic, invoke the exception handler, and resume at the next offset. Configure the target topic with `dlq`.
