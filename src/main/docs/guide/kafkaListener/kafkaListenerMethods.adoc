...
In addition to common Java types and POJOs you can also define listener methods that receive a Reactive type such as a Reactor `Mono`. For example:
...
<2> A Reactor `Mono` is used to receive the message body
<3> The `doOnSuccess` method is used to process the result

Note that in this case the method returns a rs:Publisher[] that indicates to Micronaut that the `poll` loop should continue and if `enable.auto.commit` is set to `true` (the default) the offsets will be committed potentially before the `doOnSuccess` is called.
...
@SingleResult
public Publisher<Product> receive(
    ...
}
----
