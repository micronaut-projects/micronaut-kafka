...
- Last full-suite result (micronaut-core 5.2.4, micronaut-build 8.1.3): build successful, 13 tests executed, 1 skipped (`MyTest`, see below), 0 failures.
...
## Verified with micronaut-core 5.2.4 (workarounds removed in this pass)

- `KStream.flatMapValues(lambda)` selects the `ValueMapper` overload for a one-argument Python function, so the Python
  word count uses `flatMapValues` like the Java, Kotlin and Groovy samples (the `flatMap` + `KeyValue` detour is gone).
- The `@KafkaScope` (`@ScopedProxy`) Python bean `ProductMetadata` is injected into `ProductListener` through a scoped
  proxy, so the listener no longer needs a `spec.name` `@Requires` to stay out of the other tests (it has none in Java
  either) and the whole suite runs with it active.
